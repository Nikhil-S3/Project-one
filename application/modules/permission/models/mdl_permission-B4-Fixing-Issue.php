		...
		if(count($data)>0){
			...
			foreach ($data as $key => $val) {
				...
				if($key != 'save_permissions' && $key != 'role_id'){
                    ...
                    $permission_col_name = $temp_perm2;
                    // echo $permission_col_name;exit;
					$sql = "UPDATE role_permissions SET $permission_col_name='1', updated_by='$updated_by', updated_at='$updated_at' WHERE id=$val AND role_id=$role_id";
					$this->db->query($sql);
